...
abstract class Base {
	...
	public function init_settings() {
		$this->settings = get_theme_mod( 'aengine_' . $this->id . '_settings', null );
	...
	public function registerCustomizer($wp_customize){
		$wp_customize->add_section( 'section_'.$this->id,
				array(
						'title' => __( ucfirst($this->id), AENGINE_DOMAIN ), //Visible title of section
						'priority' => 120, //Determines what order this appears in
						'capability' => 'edit_theme_options', //Capability needed to tweak
						'description' => __('Customize your header.',AENGINE_DOMAIN), //Descriptive tooltip
						'active_callback' => 'is_front_page',
				)
		);

		foreach($this->controls as $name=>$control){
			$settingName = 'aengine_' . $this->id . '_settings['.$name.']';
			...
			switch($control['type']){
				case 'color':
					$wp_customize->add_control( new \WP_Customize_Color_Control( $wp_customize, 'aengine_' . $this->id . '_settings-'.$name, array(
							'label'        => $control['label'],
					...
					break;
				case 'upload':
					$wp_customize->add_control( new \WP_Customize_Upload_Control( $wp_customize, 'aengine_' . $this->id . '_settings-'.$name, array(
							'label'        => $control['label'],
							'section'    => 'section_'.$this->id,
							'settings'   => $settingName,
					) ) );
					break;
				default:
					$wp_customize->add_control('aengine_' . $this->id . '_settings-'.$name, array(
							'label'    => $control['label'],
							'section'  => 'section_'.$this->id,
							'settings' => $settingName,
							'type'     => $control['type']
