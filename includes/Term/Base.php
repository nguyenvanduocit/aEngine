...
abstract class Base {
	...
	public function registerTerm(){
		...
		$object_types = apply_filters('aengine_term_'.$this->name.'_object_type', array());
	...
	protected function createLable () {
		...
		$labels = array(
			'name' => sprintf(_x( '%s', 'taxonomy general name', AENGINE_DOMAIN ),$plural),
			'singular_name' => sprintf(_x( '%s', 'taxonomy singular name', AENGINE_DOMAIN ), $singular),
			'search_items' => sprintf(__( 'Search %s', AENGINE_DOMAIN ), $plural),
			'all_items' => sprintf( __( 'All %s', AENGINE_DOMAIN ), $plural ),
			'parent_item' => sprintf(__( 'Parent %s', AENGINE_DOMAIN ),$singular),
			'parent_item_colon' => sprintf(__( 'Parent %s:', AENGINE_DOMAIN ), $singular),
			'new_item' => sprintf( __( 'New %s', AENGINE_DOMAIN ), $singular ),
			'update_item' => sprintf(__( 'Update %s', AENGINE_DOMAIN ), $singular),
			'add_new_item' => sprintf( __( 'Add New %s', AENGINE_DOMAIN ), $singular ),
			'new_item_name' => sprintf(__( 'New %s Name', AENGINE_DOMAIN ), $singular),
