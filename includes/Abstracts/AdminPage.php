...
//  ____________REGISTRATION COMPONENT____________
	...
	public function __construct( $file = false, $options = null ) {
		if ( is_a( $options, '\AEngine\Option' ) ) {
			...
			throw new \Exception('$options must be a \AEngine\Option');
		}
		$this->setup();
		$this->check_args();
	...
	protected function page_header() {
		...
		echo Util::html( 'h2', $this->args['page_title'] );
	}
	/**
	...
	public function admin_msg( $msg = '', $class = 'updated' ) {
		...
		echo Util::admin_notice( $msg, $class );
	}
	...
	public function table_row( $args, $formdata = false ) {
		return $this->row_wrap( $args['label'], $this->input( $args, $formdata ) );
	}
	...
	public function __call( $method, $args ) {
		...
		return call_user_func_array( array( '\AEngine\Form\FormBuilder', $method ), $args );
	}
	...
	public function js_wrap( $string ) {
		return Util::html( "script type='text/javascript'", $string );
	}
	/**
...
//  ____________INTERNAL METHODS____________
	...
	public function _action_link( $links ) {
		...
		$links[] = Util::html_link( $url, $this->args['action_link'] );
		return $links;
	}
